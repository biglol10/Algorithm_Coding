    ...
    public static int longestPath(String[] maze, Integer startRow, Integer startCol, Integer[] moveRow, Integer[] moveCol){
        ...
        while(queueX.size()>0){
            ...
            for (int i = 0; i < moveRow.length; i++) {
                ...
                if(0<=nextX && nextX< width && nextY< height && board[nextY][nextX] == -1 && maze[nextY].substring(nextX,1).equals(".")){
                    board[nextY][nextX] = board[nextY][nextX] + 1;
                    queueX.add(nextX);
                    queueY.add(nextY);
                }
            }
        ...
        for (int i = 0; i < height; i++) {
            for (int j = 0; j < width; j++) {
                if(maze[i].substring(j,1).equals(".") && board[i][j] == -1)
                    return -1;
